...
function addValidationFormat(format: string, check: (value: string) => boolean) {
    try {
...
export function addValidationFormats() {
    ...
    addValidationFormat('emoji', (value) => {
        return /^[\u{1F300}-\u{1F6FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]$/u.test(value);
    });
    ...
    addValidationFormat('ip', (value) => {
        return /^(\d{1,3}\.){3}\d{1,3}$/.test(value) || /^([0-9a-f]{1,4}:){7}[0-9a-f]{1,4}$/.test(value);
    });
}
