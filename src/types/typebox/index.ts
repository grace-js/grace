...
const Files = TypeSystem.Type<File[], Files>("Files", (options, value) => {
...
export function CompileParse<T extends TSchema>(
  schema: T,
  ...
  return (input: unknown): Static<T> => {
    ...
    throw Error(
      `${path} ${message} ${JSON.stringify(input)} ${JSON.stringify(convert)}`,
    );
  };
}
...
export { convertToBytes, validateFile } from "./validate-file";
